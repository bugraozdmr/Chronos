...
class BackgroundCore {
  ...
  BackgroundCore(this.service, this.notificationsPlugin) {
    _commandRouter = BluetoothCommandRouter(
      ...
      onGetJobs: () async {
        ...
        final jobsList = await db!.getAllJobs();
        if (jobsList.isEmpty) return "NONE";
        return jobsList.map((j) => j.name).join(',');
      },
    );
  }
  ...
  Future<void> startSession(String categoryName) async {
    ...
    final statusData = await _commandRouter.onGetStatus();
    _commandRouter.pushStatus(statusData);
  }
  ...
  Future<void> stopSession({String? reason}) async {
    ...
    _totalPausedSeconds = 0;
    _commandRouter.pushStatus("IDLE");
  }
