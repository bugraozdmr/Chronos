...
public class FlutterBluetoothSerialPlugin implements FlutterPlugin, ActivityAware {
    ...
    /// Posts results on the platform thread even when no Activity is attached
    /// (background service engine).
    private final Handler mainHandler = new Handler(Looper.getMainLooper());

    /// Obtains the BluetoothAdapter from the application context so the plugin
    /// works without an Activity (background service isolate).
    private void ensureBluetoothAdapter() {
        ...
        try {
            BluetoothManager bluetoothManager =
                    (BluetoothManager) activeContext.getSystemService(Context.BLUETOOTH_SERVICE);
                                ...
                                methodChannel.invokeMethod("handlePairingRequest", arguments, new MethodChannel.Result() {
                                    ...
                                    public void success(Object handlerResult) {
                                        ...
                                        if (handlerResult instanceof String) {
                                            ...
                                            if (activity != null) ActivityCompat.startActivity(activity, intent, null);
                                        }
...
//        if (true) throw new RuntimeException("FlutterBluetoothSerial Attached to engine");
        ...
        // The plugin should keep working even without an attached Activity
        // (e.g. inside the flutter_background_service engine). Use the
        // application context so a BluetoothAdapter can still be obtained.
...
//        if (true) throw new RuntimeException("FlutterBluetoothSerial Attached to activity");
    ...
    private void ensurePermissions(EnsurePermissionsCallback callbacks) {
        if (activity == null) {
            // No Activity (background service engine); permissions were already
            // granted while the app was in the foreground, so just verify them.
            boolean granted = ContextCompat.checkSelfPermission(activeContext,
